...
def test_old_logs_are_deleted_after_threshold(monkeypatch, tmp_path):
    ...
    # create files and set their mtimes to simulate "6 months old
    ...
    now = time.time()
    old_time = now - (six_months_seconds + 10)
    ...
    # simulated 6-month-old mtimes above; don't actually wait 6 months
    max_wait_seconds = 30
    ...
    assert (now - mtime_old1) >= six_months_seconds, "old1 mtime not old"
    assert (now - mtime_old2) >= six_months_seconds, "old2 mtime not old"
